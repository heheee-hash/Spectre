...
export function ReceiptTable({ data }: ReceiptTableProps) {
    ...
    const filteredReceipts = data.filter((rcp) =>
        rcp.id.toLowerCase().includes(search.toLowerCase()) ||
        rcp.reference.toLowerCase().includes(search.toLowerCase()) ||
        (rcp.vendorName || "").toLowerCase().includes(search.toLowerCase())
    );
                ...
                <Table>
                    ...
                    <TableBody>
                        {filteredReceipts.map((rcp) => (
                            <TableRow key={rcp.id}>
                                <TableCell className="font-medium font-mono text-primary">{rcp.id.substring(0, 8)}</TableCell>
                                <TableCell>{rcp.reference}</TableCell>
                                <TableCell>{rcp.vendorName || "N/A"}</TableCell>
                                <TableCell>{new Date(rcp.createdAt).toLocaleDateString()}</TableCell>
                                <TableCell>
                                    <Badge variant="secondary" className={`capitalize ${getStatusColor(rcp.status)}`}>
                                        {rcp.status.toLowerCase()}
                                    </Badge>
                                </TableCell>
                                    ...
                                    <DropdownMenu>
                                        ...
                                        <DropdownMenuContent align="end">
                                            ...
                                            <DropdownMenuItem asChild>
                                                <Link href={`/receipts/${rcp.id}`}><Eye className="mr-2 h-4 w-4" /> View Details</Link>
                                            </DropdownMenuItem>
                                            {isManager && rcp.status !== "DONE" && (
                                                <DropdownMenuItem asChild>
                                                    <Link href={`/receipts/${rcp.id}/edit`}><FileEdit className="mr-2 h-4 w-4" /> Edit Receipt</Link>
                                                </DropdownMenuItem>
                                            )}
